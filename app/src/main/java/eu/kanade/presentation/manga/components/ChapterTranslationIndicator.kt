...
fun ChapterTranslationIndicator(
    ...
    when (val translationState = translationStateProvider()) {
        ...
        ChapterTranslation.State.QUEUE,
        ChapterTranslation.State.WAKING_SERVER,
        ChapterTranslation.State.WAITING_SERVER,
        ChapterTranslation.State.TRANSLATING,
        ...
        ChapterTranslation.State.ERROR -> ErrorIndicator(
            enabled = enabled,
            modifier = modifier,
            onClick = onClick,
        )
...
private fun TranslatingIndicator(
    ...
    Box(
        ...
        val translatedPages = translatedPagesProvider()
        val totalPages = totalTranslationPagesProvider()
        val progress = if (totalPages > 0) {
            translatedPages.toFloat() / totalPages.toFloat()
        } else {
            null
        }
